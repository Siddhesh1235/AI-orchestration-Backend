...
def test_voice_stt_endpoint_with_audio_file():
    ...
    mock_res = {"success": True, "language": "mr", "text": "तक्रार", "engine": "faster-whisper (small)"}
    with patch("app.api.routes.voice.local_stt_service.transcribe_audio", return_value=mock_res):
        ...
        assert any(eng in json_data["engine"] for eng in ["Bhashini", "faster-whisper", "whisper"])
