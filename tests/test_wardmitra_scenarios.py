...
def test_scenario_16_smart_multi_complaint_switching_and_duplicate_reset():
    """
    TEST 16:
    Multi-complaint flow with duplicate reset, smart category switching, and explicit new complaint intent.
    Ensures that when a user moves from garbage -> pothole -> water leakage, the bot does NOT stay stuck on garbage!
    """
    db = SessionLocal()
    session_id = f"test-multi-turn-{random.randint(1000, 9999)}"
    try:
        # Turn 1: Garbage complaint with location
        t1 = conversational_service.handle_chat(
            db=db,
            message="रस्त्यावर खूप कचरा साचला आहे आणि दुर्गंधी येत आहे, कृष्णा चौक सांगवी",
            session_id=session_id
        )
        assert t1["category"] == "garbage"
        assert "कचरा" in t1["reply"]

        # Turn 2: Confirm registration
        t2 = conversational_service.handle_chat(
            db=db,
            message="हो",
            session_id=session_id
        )
        assert t2["intent"] in ["COMPLAINT_REGISTERED", "DUPLICATE_DETECTED"]

        # Turn 3: User introduces a completely NEW problem: Pothole
        t3 = conversational_service.handle_chat(
            db=db,
            message="मुख्य रस्त्यावर मोठा खड्डा पडला आहे, वाहने घसरून पडत आहेत",
            session_id=session_id
        )
        # CRITICAL ASSERTION: MUST NOT BE GARBAGE! Must be pothole!
        assert t3["category"] == "pothole", f"Expected 'pothole', but got {t3['category']}"
        assert any(w in t3["reply"] for w in ["खड्ड", "रस्ते", "खड्डे"])

        # Turn 4: User switches to Water Pipeline Leakage
        t4 = conversational_service.handle_chat(
            db=db,
            message="पिण्याच्या पाण्याची पाईपलाईन फुटून रस्त्यावरून पाणी वाहत आहे",
            session_id=session_id
        )
        # CRITICAL ASSERTION: MUST NOT BE POTHOLE OR GARBAGE! Must be water leakage!
        assert t4["category"] in ["pipeline_water_leakage", "water_supply"], f"Expected water category, but got {t4['category']}"
        assert any(w in t4["reply"] for w in ["पाणी", "पाण्या", "पाईपलाईन", "जल"])

        # Turn 5: User explicitly asks to start a new complaint
        t5 = conversational_service.handle_chat(
            db=db,
            message="नवीन तक्रार नोंदवा",
            session_id=session_id
        )
        assert t5["intent"] == "START_NEW_COMPLAINT"
        assert t5.get("buttons") is not None
        assert len(t5["buttons"]) >= 4

        print("✓ TEST 16 Passed: Smart category switching and duplicate state reset successfully verified!")
    finally:
        db.close()
...
if __name__ == "__main__":
    print("\n=======================================================")
    print("RUNNING WARDMITRA AI SCENARIO VERIFICATION TESTS (1-16)")
    print("=======================================================\n")
    ...
    test_scenario_16_smart_multi_complaint_switching_and_duplicate_reset()
    print("\n🎉 ALL 16 SCENARIO TESTS PASSED SUCCESSFULLY!\n")
